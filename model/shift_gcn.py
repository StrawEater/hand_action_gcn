...
class Shift_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3):
        ...
        # derive num_point from the adjacency matrix so this class works for any graph
        num_point = A.shape[1]
        ...
        index_array = np.empty(num_point * out_channels, dtype=np.int64)
        for i in range(num_point):
    ...
    def forward(self, x0):
        ...
        x = self.bn(x)
        x = x.view(n, t, v, self.out_channels).permute(0, 3, 1, 2).contiguous() # n,c,t,v
...
class ReconHead(nn.Module):
    """Upsamples backbone features back to input resolution and projects to input channels."""
    def __init__(self, feat_channels=256, out_channels=3):
        super(ReconHead, self).__init__()
        self.proj = nn.Sequential(
            nn.Conv2d(feat_channels, 64, 1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, out_channels, 1),
        )

    def forward(self, x, target_t, target_v):
        # x: (N*M, feat_channels, T/4, V) → (N*M, out_channels, target_t, target_v)
        x = F.interpolate(x, size=(target_t, target_v), mode='bilinear', align_corners=False)
        return self.proj(x)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3):
        ...
        self.recon_head = ReconHead(feat_channels=256, out_channels=in_channels)
    ...
    def forward(self, x, return_recon=False):
        ...
        if return_recon:
            ...
            recon = r.view(N, M, self._in_channels, T, V)
            recon = recon.permute(0, 2, 3, 4, 1).contiguous()     # (N, C_in, T, V, M)
